...
If your source system can restart an already ended subscription _(Wiedereinweisung)_, you can't reset `endDate`. In plenigo you have to create a new subscription. It will use same `xternalSystemId` and will be added to the chain - thats why it will use same `chainId`.
```php
    // @see https://api.plenigo.com/#tag/Order-Imports/operation/orderImport
    $import = [
        "suppressMail" => true, // set to true if you do not want to send process emails
